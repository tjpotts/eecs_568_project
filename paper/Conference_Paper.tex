...
Many of today's autonomous and semi-autonomous vehicles rely heavily on the ability to localize correctly to a known map. As autonomous vehicles expand their range to include metropolitan areas, they can experience decreased GPS accuracy due to the multipath effects in the GPS signal, which can cause errors in the localization. We h  We have developed a solution that uses a factor graph based SLAM method  Our solution to this challenge is to detect traffic signs and use them for localizing to a known map of traffic signs.  We use slam to construct a local traffic sign point cloud.  Then we use a Semantic Iterative Closest Point algorithm to correlate the local point cloud to a global point cloud.
